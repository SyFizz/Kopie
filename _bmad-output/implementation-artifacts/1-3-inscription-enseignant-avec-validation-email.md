...
- [x] [Review][Patch] Renvoyer `400 INVALID_OR_EXPIRED_TOKEN` pour les tokens fournis mais malformés, afin de respecter l'AC4 ; garder `422` uniquement pour le paramètre manquant si souhaité [`apps/api/app/api/v1/endpoints/auth.py`:31] — Retrait des `min_length=16, max_length=128` sur la `Query` ; tout token *fourni* mais non reconnu (incluant trop court / trop long / caractères invalides) est désormais mappé à 400 `INVALID_OR_EXPIRED_TOKEN` par `AuthService.verify_email`. Le contrat OpenAPI a été nettoyé en conséquence : retrait des bornes du schéma du paramètre, retrait de la réponse `422` documentée pour `verify-email`, description du 400 enrichie (« malformé »). Le test `test_verify_email_token_too_short_rejected` est devenu `test_verify_email_token_malformed_returns_400` et boucle sur 4 variantes (`"a"`, `"short"`, `"x"*200`, `"###%%%###"`).
- [x] [Review][Patch] Préserver la garde anti-StrictMode tout en ignorant les réponses obsolètes si le query param `token` change pendant un appel [`apps/web-prof/src/features/auth/VerifyEmailPage.tsx`:24] — Combinaison de deux refs : `dispatchedTokenRef` (un seul dispatch par valeur de `token` vue → neutralise le double-mount StrictMode) et `currentTokenRef` (toujours mis au `token` courant de l'effet le plus récent → à la résolution du fetch, on compare la valeur capturée en closure et on ignore si elle ne correspond plus). Nouveau test « ignore les réponses obsolètes si le token change pendant un appel » : fetch A *en attente*, navigation vers `?token=B`, résolution de B en 400 → écran « Lien invalide » ; on résout A après coup en succès → l'écran reste « Lien invalide ».
- [x] [Review][Patch] Renforcer le test anti-fuite de token pour capturer réellement les logs `structlog.PrintLoggerFactory` et vérifier qu'au moins un log est émis [`apps/api/tests/test_email_service.py`:25] — `caplog` ne voit pas les enregistrements émis par `structlog.PrintLoggerFactory` (sortie stdout directe), le test passait donc trivialement sur une liste vide. Remplacement par `structlog.testing.capture_logs()` + trois assertions : (1) `cap_logs` n'est pas vide (assertion positive : au moins un log émis), (2) ni le token complet ni l'URL n'apparaissent dans la sérialisation JSON des records, (3) un `token_prefix` ≤ 9 caractères, terminé par `…` et préfixe authentique du token est présent.
...
| 2026-05-19 | BMad (dev-story re-patch) | Seconde passe : (1) token malformé → 400 `INVALID_OR_EXPIRED_TOKEN` au lieu de 422 pour respecter l'AC4 ; bornes runtime et contrat OpenAPI nettoyés ; test paramétré sur 4 variantes ; (2) `VerifyEmailPage` garde la dédup StrictMode (`dispatchedTokenRef`) tout en ignorant les réponses obsolètes lors d'un changement de token via `currentTokenRef` ; nouveau test de race « token A en vol, navigation vers B, B répond 400, A résout en succès → l'écran reste 400 » ; (3) test anti-fuite token passe par `structlog.testing.capture_logs()` + assertion positive (au moins un log émis) + vérification du préfixe ≤ 9 chars. 21 tests pytest et 13 tests vitest verts ; ruff/mypy/build/lint clean. Status → review. |
